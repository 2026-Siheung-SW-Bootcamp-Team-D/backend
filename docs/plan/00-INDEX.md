...
| 기준 문서 | `../../../docs/specs/기능명세서_v1.3.md`, `API명세서_v1.1.md`, `ERD_v1.0.md`, `시스템아키텍처_v1.0.md`, `화면연동명세서_v1.0.md` |
| 범위 | **백엔드 한정.** P7 계약 동결 후 프론트엔드 실제 연동 시작 |
...
| [`11-agent-prompts.md`](11-agent-prompts.md) | — | 단계별 구현 에이전트 실행 프롬프트 |
| [`12-phase7-canonical-alignment.md`](12-phase7-canonical-alignment.md) | P7 | 완료된 P0~P6를 최신 후보 보드·화면 연동 계약에 정렬 |

---

## 2. 현재 상태 (2026-07-24 기준 실측)

P0~P6 구현과 V1·V2 migration은 완료되어 있다. P7은 이를 다시 구현하는 단계가 아니라, 최신 `docs/specs`와 실제 백엔드 사이의 다음 차이를 해소하는 **프론트 연동 전 마지막 보정 단계**다.

- 유지: 보드·참여자·출발지, 후보 장소, 댓글, 다중 장소 좋아요, 현재 선택 장소, ODsay 지역 작업
- 수정: 검색 경로와 DTO, 댓글 필드명, Kakao 검색 개수, 자유 좌표 주변 검색, 지역 결과 구조와 기준점 선정
- 격리: 과거 Vote·Course·Departure 코드와 테이블은 삭제하지 않고 신규 MVP API에서 기본 비활성
- migration: V1·V2는 불변이며 지역 저장 모델 변경은 V3에서 수행
- 완료 조건: P7 OpenAPI가 `화면연동명세서_v1.0.md`와 일치한 뒤 FE가 실제 API 연동을 시작
...
| P7 | P0~P6 | 기존 동작을 보존하면서 최신 후보 보드·검색·지역 결과 계약으로 일괄 정렬한다 |
...
### Wave 6 — 현재 작업
P7 최신 계약 정렬을 단독 수행한다. P0~P6 산출물과 V1·V2 migration은 되돌리지 않으며, [`12-phase7-canonical-alignment.md`](12-phase7-canonical-alignment.md)의 V7-1~V7-12를 모두 통과하기 전에는 FE 실제 연동을 시작하지 않는다.

---

## 5. 단계별 완료 게이트 요약

P0~P6은 아래 공통 4종을 기준으로 완료됐다. P7은 이 기준에 더해 [`12-phase7-canonical-alignment.md`](12-phase7-canonical-alignment.md)의 canonical OpenAPI·주변 검색·지역 중심·레거시 격리·FE handoff 게이트를 모두 통과해야 한다.
...
```
P0 → P1 → P2 → (P3 ∥ P4 ∥ I2) → P5 → P6 → P7 → FE 실제 연동
       └ I0 → I1 (P1~P2와 병렬) ────────┘ → I3
```
...
- 프론트엔드 구현. P7 완료 뒤 [`10-fe-contract-sync.md`](10-fe-contract-sync.md)와 화면 연동 명세를 기준으로 별도 착수한다.
...
## 10. P7 전환 결정 이력 (2026-07-24)

| # | 기존 상태 | 최신 결정 | P7 처리 |
|---:|---|---|---|
| 1 | 정식 Vote·Course·Departure API가 남아 있음 | 신규 MVP는 좋아요와 현재 선택만 사용 | 코드는 보존하고 기본 API·OpenAPI에서 격리 |
| 2 | Kakao 검색이 5개·레거시 경로로 제한 | 검색 자체가 핵심 기능이며 FE DTO를 공급자 중립 형태로 고정 | canonical 검색 경로·DTO, 최대 15개, 자유 좌표 주변 검색 |
| 3 | 공통 영역 조각 내부 역을 우선 추천 | 공정성보다 “어디서 놀지 탐색할 시작점”이 중요 | 참여자 대표 중심 주변 교통 기준점을 거리순 제시 |
| 4 | 교집합 또는 기준점 부재가 실패로 이어질 수 있음 | 도달시간·공통 영역은 참고값이며 오차 허용 | `commonArea=null`, `anchors=[]`도 정상 성공 |
| 5 | `area_candidate` 명칭이 후보 장소와 혼동 | 지역 결과는 장소 후보가 아니라 탐색 기준점 | V3에서 `area_suggestion`으로 정렬 |

상세 구현 순서와 완료 기준은 [`12-phase7-canonical-alignment.md`](12-phase7-canonical-alignment.md)를 단일 실행 원본으로 사용한다.
