 ...
 *
 * DEPRECATED: 투표 기능은 P6의 후보 장소 기반 흐름(좋아요, 댓글, 선택)으로 대체되었습니다.
 * 기존 엔드포인트는 하위 호환성을 위해 유지되며, 새로운 개발에서는 사용하지 않습니다.
 */
@Deprecated(message = "투표 기능은 P6의 후보 장소 기반 흐름으로 대체되었습니다. 새로운 개발에서는 사용하지 않습니다.")
@org.springframework.boot.autoconfigure.condition.ConditionalOnProperty(prefix = "app", name = arrayOf("legacy-api-enabled"), havingValue = "true")
@RestController
...
class VoteController(
    ...
    @PostMapping
    @Operation(summary = "투표 생성", description = "호스트가 장소 투표를 생성한다. 보드당 열린 투표는 1개만 허용.", deprecated = true)
    fun create(
    ...
    @GetMapping
    @Operation(summary = "투표 목록 조회", description = "status=OPEN|CLOSED 필터와 페이지네이션을 지원한다.", deprecated = true)
    fun list(
        @PathVariable boardId: String,
        @CurrentParticipant principal: ParticipantPrincipal,
    ...
    @GetMapping("/{voteId}")
    @Operation(summary = "투표 상세 조회", description = "투표의 후보와 투표 결과를 조회한다. 익명 투표는 참여자 신원을 포함하지 않는다.", deprecated = true)
    fun getDetail(
        @PathVariable boardId: String,
    ...
    @Operation(summary = "내 투표 교체", description = "내 투표를 생성하거나 전체 교체한다. 빈 배열은 투표 취소를 의미한다.", deprecated = true)
    fun updateMyBallot(
        @PathVariable boardId: String,
        @PathVariable voteId: String,
        @CurrentParticipant principal: ParticipantPrincipal,
    ...
    @PatchMapping("/{voteId}")
    @Operation(summary = "투표 조기 종료", description = "호스트가 투표를 조기 종료한다. 이미 종료된 투표는 200으로 반환 (멱등).", deprecated = true)
    fun close(
        @PathVariable boardId: String,
        @PathVariable voteId: String,
        @CurrentParticipant principal: ParticipantPrincipal,
