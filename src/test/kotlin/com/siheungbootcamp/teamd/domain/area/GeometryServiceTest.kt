...
class GeometryServiceTest {
    ...
    @Test
    fun `intersectionAreaKm2는 실제 평방 킬로미터 면적을 계산한다`() {
        // 작은 테스트 직사각형: 대략 0.1도 x 0.1도 (적도 근처)
        // 위도 차이 0.1도 ~ 11 km, 경도 차이 0.1도 ~ 9 km (적도에서)
        // 예상 면적: ~ 100 km²
        val coordinates = arrayOf(
            Coordinate(127.0, 37.0),    // 시작점
            Coordinate(127.1, 37.0),    // 동쪽 0.1도
            Coordinate(127.1, 37.1),    // 북동쪽
            Coordinate(127.0, 37.1),    // 북쪽
            Coordinate(127.0, 37.0)     // 시작점으로 돌아옴
        )
        val ring = geometryFactory.createLinearRing(coordinates)
        val polygon = geometryFactory.createPolygon(ring)

        val areaKm2 = service.intersectionAreaKm2(polygon)

        // 면적이 양수이고 합리적인 범위(대략 80~150 km²)에 있어야 함
        assertTrue(areaKm2 > 0.0, "면적은 양수여야 함")
        assertTrue(areaKm2 < 200.0, "예상 범위(~100 km²)를 초과하면 안 됨")
    }

    @Test
    fun `intersectionAreaKm2는 빈 폴리곤에 대해 0을 반환한다`() {
        val emptyGeometry = geometryFactory.createPolygon()
        val areaKm2 = service.intersectionAreaKm2(emptyGeometry)
        assertTrue(areaKm2 == 0.0, "빈 폴리곤은 면적 0")
    }

    @Test
    fun `intersectionAreaKm2는 음수 면적을 방지한다`() {
        // 시계 방향 좌표 (음수 면적을 만들 수 있음)
        val coordinates = arrayOf(
        ...
        val areaKm2 = service.intersectionAreaKm2(polygon)
        assertTrue(areaKm2 >= 0.0, "면적은 음수일 수 없음")
    }
}
